...
const conventionsName = 'nx-preset-ts-monorepo';
...
describe(`e2e: ${conventionsName}`, () => {
  ...
  it('should build successfully', async () => {
    ...
    const resultApp = runCommand(`nx build my-package --skip-sync`, wsCwd);
    expect(stripAnsi(resultApp)).toContain(`Successfully ran target build for project my-package`);
  });
});
